...
export async function GET(req: NextRequest) {
  ...
  const blocked = enforceRateLimit("ping", extractClientIp(req), req, rateHeaders, CORS)
  ...
  if (!apiKey || !apiKey.startsWith("bsm_live_")) {
    return NextResponse.json({ ok: false, error: "Missing or invalid API key" }, { status: 401, headers: { ...CORS, ...rateHeaders } })
  }
  ...
  if (!keyRow || !keyRow.is_active) {
    return NextResponse.json({ ok: false, error: "Invalid or revoked API key" }, { status: 401, headers: { ...CORS, ...rateHeaders } })
  }
  const { data: prof } = await db.from("profiles").select("full_name, email, status").eq("id", keyRow.user_id).maybeSingle()
  return NextResponse.json({ ok: true, account: { name: prof?.full_name ?? null, email: prof?.email ?? null, status: prof?.status ?? null } }, { headers: { ...CORS, ...rateHeaders } })
}
