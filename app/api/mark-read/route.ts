...
import { markAsRead } from "@/lib/evolution"

export async function OPTIONS() { return new NextResponse(null, { status: 204, headers: CORS }) }

// POST { remote_jid, message_id, from_me?, instance_id? }
export async function POST(req: NextRequest) {
  ...
  if (!b.remote_jid || !b.message_id) return NextResponse.json({ error: "'remote_jid' and 'message_id' are required" }, { status: 400, headers: CORS })
  try {
    const r = await markAsRead(a.inst.instance_name, b.remote_jid, b.message_id, b.from_me ?? false)
